...
def production():
    ...
    env.hosts = ['46.101.181.20']
    env.shell = 'bash -c'
    env.use_ssh_config = True    #импортируем конфигурацию нашего ssh-клиента
    env.project_user = "app42"    #пользователь, от которого работает проект на удаленном сервере
    env.sudo_user = env.project_user
    env.base_dir = "/home/42"    #базовая директория для проекта
